...
def draw_circle (ct,x,y,r):
  ct.set_line_width (2.0)
  ct.set_source_rgb (*fg_nigre)
  ct.arc (x,y,r,0,tau)
  ct.stroke ()

def draw_cross (ct,x,y,tick_r):
  ct.set_line_width (2.0)
  ct.set_source_rgb (*fg_nigre)
...
def draw_tick (ct,x0,y0,r1,r2,phi):
  pt1 = add ((x0,y0), polar_xy (r1,phi))
  pt2 = add ((x0,y0), polar_xy (r2,phi))
  ct.move_to (*pt1)
  ct.line_to (*pt2)

def draw_ticks (ct,center_x,center_y,circle_r):
  ct.set_line_width (2.0)
  ct.set_source_rgb (*fg_nigre)
  ...
  for i,a in enumerate (ranges):
    a = a - rot_zero
    # print (i,a,b)
    draw_tick (ct,center_x,center_y,circle_r-5,circle_r+5,a)
  ct.stroke ()

def centered_text (ct,x,y,s):
  r = 32
...
def draw_number (ct,x,y,s,r=29,y0=None):
...
def draw_square (ct,x,y,r=29,y0=None):
  ct.rectangle (x-r,y-r,2*r,2*r)
  ct.fill ()

def draw_chess_board (ct):
  ...
  for x in range (0,8):
    for y in range (0,8):
      ...
      draw_square (ct,x1,y1)

def draw_decimals (ct,m):
  draw_chess_board (ct)
  if len (tau_digits) > m:
    for n in range (0,m):
      s = tau_digits [n+1]
      ...
      if n == 0:
        draw_number (ct,x,y,"6.",r=29)
      else:
        x0,y0 = draw_number (ct,x,y,s,r=29)

def draw_r (ct,center_x,center_y,r1,r2):
  offset = (r2-r1) % 10
  ct.set_line_width (2.0)
...
def draw_frame (ct,i):
  ...
  for ii in range (0,int(decs+1)):
    ...
    if ii == 0 and not condensed2:
      ...
      #print (r)
      draw_circle (ct,cx-r, cy, r)
      draw_archie (ct,cx-r, cy, r+10,4)
      if r > 20:
        draw_r (ct,cx-r, cy, 7, r-7)
      draw_cross (ct,cx-r, cy, 5)
      if r > 30:
        draw_ticks (ct,cx-r, cy, r)
    if p1 < 999999 and p2 < 999999:
      rainbow (ct,x0+rbw*ii, cy+p1, x0+rbw*(ii+1), cy-p2)
    else:
      ct.set_source_rgb (*colordict[s[0]])
      rect1 (ct,x0+rbw*ii,0,x0+rbw*(ii+1),h_pic)
  draw_decimals (ct,ii+1)
  #filename = f"pics/phase-{i}.png"
...
def draw_window (self, widget, ct):
  draw_frame (ct,int(self.frame))

# ffmpeg -y -f concat -i mylist.txt -c:v libx264 -pix_fmt yuv420p tau-001.mp4
def slider_changed (self,e):
  # print (dir(e))
  self.frame = self.scale.get_value ()
...
def init_ui (self):
  ...
  self.darea.set_size_request(1280, 720)
  vbox.pack_start(self.darea, True, True, 0)
  self.resize (1280, 760)
  self.scale = Gtk.Scale.new_with_range ( 
    orientation=Gtk.Orientation.HORIZONTAL, min=0, max=6000, step=1)
  self.scale.set_size_request(1280, 40)
  self.scale.connect ('value-changed', self.on_slider_change)
...
class Win (Gtk.Window):
  ...
  def on_draw (self, widget, ct):
    draw_window (self, widget, ct)
  def on_scroll (self,w,e):
    scrolling (self,w,e)
    return True
  def on_slider_change (self,event):
    slider_changed (self,event)
